...
const replayManagementBridge = `;{
  const pvEntries=(0,R.gn)(pvOwner,jp,"f"),pvMaxReplays=2000,pvWorker=(0,R.gn)(pvOwner,Ip,"f"),pvTrack=(0,R.gn)(pvOwner,Lp,"f"),pvMountains=(0,R.gn)(pvOwner,Dp,"f"),pvStart=pvTrack.getStartTransform();
  ...
  for(let pvIndex=0;pvIndex<pvEntries.length;pvIndex++){
    ...
    pvEntry.polyviewerVisible=!0;pvEntry.polyviewerPreviewVisible=!0;pvEntry.polyviewerOpacity=1;pvEntry.polyviewerManaged=!1;pvEntry.polyviewerSimulationState="native";
  }
  let pvNextReplayId=1;
  const pvCreateReplayStore=()=>{const pvStore=window.__POLYVIEWER_CREATE_PACKED_REPLAY_STORE__?.();if(!pvStore)throw new Error("PolyViewer replay storage is not ready. Reload once and try again.");return{get polyviewerPackedBytes(){return pvStore.packedBytes},push:pvState=>pvStore.push(pvState),pushPacked:pvBytes=>pvStore.pushPacked(pvBytes),getFrame:pvFrame=>pvStore.getFrame(pvFrame),getLastFrame:()=>new bt.A(pvStore.lastFrame)}};
  const pvIsReady=pvEntry=>pvEntry.polyviewerManaged?pvEntry.polyviewerSimulationState==="ready":pvEntry.replay.getLastFrame().numberOfFrames>=pvReplay.durationFrames;
  const pvResetSimulation=pvEntry=>{if(!pvEntry.polyviewerManaged)return;if(null!=pvEntry.carId){pvWorker.deleteCar(pvEntry.carId);pvEntry.carId=null}pvEntry.polyviewerSimulationGeneration+=1;const pvStore=pvCreateReplayStore();pvStore.push(pvEntry.polyviewerInitialState);pvEntry.replay=pvStore;pvEntry.polyviewerSimulationState="idle"};
  const pvStartSimulation=pvEntry=>{if(!pvEntry.polyviewerManaged||pvEntry.polyviewerSimulationState!=="idle")return;const pvGeneration=++pvEntry.polyviewerSimulationGeneration,pvStore=pvCreateReplayStore(),pvCallback=pvBytes=>{if(pvEntry.polyviewerSimulationGeneration!==pvGeneration)return;pvStore.pushPacked(pvBytes);const pvFrame=pvBytes[0]|pvBytes[1]<<8|pvBytes[2]<<16;if(null!=pvEntry.carId&&pvFrame>=pvReplay.durationFrames){pvWorker.deleteCar(pvEntry.carId);pvEntry.carId=null;pvEntry.polyviewerSimulationState="ready";pvReplay.refreshPerformance?.()}};pvCallback.polyviewerPacked=!0;const pvCreated=pvWorker.createCar(pvStart,pvMountains.getMountainVertices(),pvMountains.getMountainOffset(),(0,R.gn)(pvOwner,Np,"f"),pvEntry.polyviewerRecording,pvCallback);pvEntry.polyviewerInitialState=pvCreated.carState;pvStore.push(pvCreated.carState);pvEntry.replay=pvStore;pvEntry.carId=pvCreated.id;pvEntry.polyviewerSimulationState="simulating";pvWorker.startCar(pvCreated.id,new bt.A(pvReplay.durationFrames))};
  pvReplay.syncPreviewSimulations=()=>{if(pvReplay.renderPreparing||pvReplay.renderPrepared||pvReplay.renderMode)return;for(const pvEntry of pvEntries)if(pvEntry.polyviewerManaged)pvEntry.polyviewerPreviewVisible?pvStartSimulation(pvEntry):pvEntry.polyviewerSimulationState!=="idle"&&pvResetSimulation(pvEntry)};
  pvReplay.refreshVisibility=()=>{
    const pvEnabled=pvEntries.filter(pvEntry=>pvEntry.polyviewerVisible!==!1),pvPreview=new Set(pvEnabled.slice(0,pvReplay.previewLimit).map(pvEntry=>pvEntry.polyviewerId)),pvPriority=pvEntries.find(pvEntry=>pvEntry.polyviewerId===pvReplay.priorityReplayId&&pvEntry.polyviewerVisible!==!1);
    ...
    for(const pvEntry of pvEntries){const pvActual=pvEntry.polyviewerVisible!==!1&&(pvReplay.renderMode||pvPreview.has(pvEntry.polyviewerId));if(pvEntry.polyviewerPreviewVisible!==pvActual)pvEntry.polyviewerPreviewVisible=pvActual,pvEntry.car.setVisible(pvActual)}
    pvReplay.syncPreviewSimulations()
  };
  pvReplay.refreshPerformance=()=>{const pvVisible=pvEntries.filter(pvEntry=>pvEntry.polyviewerVisible!==!1),pvPreview=pvVisible.filter(pvEntry=>pvEntry.polyviewerPreviewVisible!==!1),pvReady=pvVisible.filter(pvIsReady).length,pvPackedBytes=pvEntries.reduce((pvTotal,pvEntry)=>pvTotal+(pvEntry.replay.polyviewerPackedBytes??0),0);pvReplay.performanceStatus={quality:"full",totalReplays:pvEntries.length,visibleReplays:pvVisible.length,previewReplays:pvPreview.length,packedBytes:pvPackedBytes,readyReplays:pvReady,renderReady:pvVisible.length>0&&pvReady===pvVisible.length};return pvReplay.performanceStatus};
  ...
  pvReplay.setPriorityReplay=pvId=>{if(pvEntries.some(pvEntry=>pvEntry.polyviewerId===pvId)){pvReplay.priorityReplayId=pvId;pvReplay.refreshVisibility();pvReplay.refreshPerformance()}};
  ...
  pvReplay.addReplay=(pvRecordingString,pvName,pvMetadata={})=>{
    if(pvEntries.length>=pvMaxReplays)throw new Error("PolyViewer currently supports up to "+pvMaxReplays+" imported cars.");
    ...
    let pvCarStyle=pvMain.car.getCarStyle();if("string"==typeof pvMetadata.carStyle&&pvMetadata.carStyle.trim().length>0)pvCarStyle=pvCarStyle.constructor.deserializeSafe(pvMetadata.carStyle.trim());
    const pvCar=new U.A(null,pvStart,pvRecording,null,(0,R.gn)(pvOwner,Gp,"f"),(0,R.gn)(pvOwner,Fp,"f"),pvMountains,pvTrack,(0,R.gn)(pvOwner,Np,"f"),(0,R.gn)(pvOwner,Wp,"f"),null);pvCar.setCarStyle(pvCarStyle);
    const pvInitialState=pvMain.replay.getFrame(0);if(null==pvInitialState)throw new Error("Main replay start state is unavailable");const pvPackedReplay=pvCreateReplayStore();pvPackedReplay.push(pvInitialState);
    const pvEntry={replay:pvPackedReplay,checkpointTimes:[],finishSpeed:null,carId:null,car:pvCar,settings:{...pvMain.settings,nickname:"string"==typeof pvName&&pvName.trim().length>0?pvName.trim():"Replay "+(pvEntries.length+1),recording:pvRecording,time:new bt.A(pvReplay.durationFrames),carStyle:pvCarStyle},polyviewerId:"replay-"+pvNextReplayId++,polyviewerName:"",polyviewerVisible:!0,polyviewerPreviewVisible:!1,polyviewerOpacity:1,polyviewerVerifiedState:Number.isSafeInteger(pvMetadata.verifiedState)?pvMetadata.verifiedState:null,polyviewerManaged:!0,polyviewerRecording:pvRecording,polyviewerInitialState:pvInitialState,polyviewerSimulationState:"idle",polyviewerSimulationGeneration:0};
    pvEntry.polyviewerName=pvEntry.settings.nickname;pvCar.setNameTag(null,pvEntry.polyviewerName);pvEntries.push(pvEntry);if(pvReplay.replayBatchDepth===0)pvReplay.refreshVisibility(),pvReplay.refreshPerformance();else{const pvPreviewCount=pvEntries.filter(pvItem=>pvItem.polyviewerPreviewVisible!==!1&&pvItem.polyviewerVisible!==!1).length,pvPreviewEnabled=pvPreviewCount<pvReplay.previewLimit;pvEntry.polyviewerPreviewVisible=pvPreviewEnabled;pvCar.setVisible(pvPreviewEnabled);pvPreviewEnabled&&pvStartSimulation(pvEntry)}return{id:pvEntry.polyviewerId,name:pvEntry.polyviewerName,visible:!0,opacity:1,nameTagVisible:!1,removable:!0}
  };
  ...
  pvReplay.setReplayVisible=(pvId,pvVisible)=>{const pvEntry=pvEntries.find(pvItem=>pvItem.polyviewerId===pvId);if(!pvEntry)throw new Error("Replay not found");pvEntry.polyviewerVisible=!!pvVisible;pvReplay.refreshVisibility();pvReplay.refreshPerformance()};
