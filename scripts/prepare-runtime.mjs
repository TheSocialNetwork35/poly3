...
const replayManagementBridge = `;{
  ...
  pvReplay.renderMode=!1;pvReplay.previewLimit=20;
  for(let pvIndex=0;pvIndex<pvEntries.length;pvIndex++){
    ...
    pvEntry.polyviewerVisible=!0;pvEntry.polyviewerPreviewVisible=!0;pvEntry.polyviewerOpacity=1;
  }
  ...
  pvReplay.setRenderMode=pvRendering=>{pvReplay.renderMode=!!pvRendering;pvReplay.refreshVisibility();pvReplay.refreshPerformance()};
  pvReplay.refreshPerformance=()=>{
    const pvVisible=pvEntries.filter(pvEntry=>pvEntry.polyviewerVisible!==!1);
    const pvPreview=pvEntries.filter(pvEntry=>pvEntry.polyviewerPreviewVisible!==!1&&pvEntry.polyviewerVisible!==!1);
  ...
  pvReplay.addReplay=(pvRecordingString,pvName,pvMetadata={})=>{
    ...
    pvEntry.replay.push(pvCreated.carState);pvEntry.carId=pvCreated.id;pvEntries.push(pvEntry);pvReplay.refreshVisibility();pvReplay.refreshPerformance();pvWorker.startCar(pvCreated.id,new bt.A(pvRequestedFrames));return pvReplay.listReplays().at(-1)
  };
