...
describe("Blender vehicle lighting metadata", () => {
  it("exports each visible car's independent chassis pose and real brake state", () => {
  ...
  it("reads the native animated chassis and brake flag without guessing from the brake key", () => {
    ...
    expect(car.polyviewerGetExportCar()).toEqual({ matrix, braking: false, visible: true });
    car.state.brakeLightEnabled = true;
    car.group.visible = false;
    expect(car.polyviewerGetExportCar()).toEqual({ matrix, braking: true, visible: false });
  });
});
