...
Keep this script beside scene.json, frames/ and resources/. If its path is lost,
select scene.json in the file browser. Saves no files automatically.
"""
import bpy
import json
import math
...
def find_archive(script_path, text_path=None):
    # Blender Text Editor can use a synthetic __file__ such as /import_scene.py.
    # Only use explicit script/text paths, never guess from unrelated scenes.
    for value in (text_path, script_path):
        if not value:
            continue
        root = Path(bpy.path.abspath(str(value))).resolve().parent
        if (root / 'scene.json').is_file() and (root / 'frames').is_dir():
            return root
    return None
...
def import_archive(root):
    ...
    if not (ROOT / 'scene.json').is_file() or not (ROOT / 'frames').is_dir():
        raise RuntimeError('Extract the entire export ZIP, then select scene.json beside frames/ and resources/.')
    ...
    for frame in range(1, DATA['frameCount'] + 1):
        snapshot = json.loads((ROOT / 'frames' / ('%06d.json' % (frame-1))).read_text(encoding='utf-8'))
...
def register_archive_picker():
    from bpy_extras.io_utils import ImportHelper

    class POLYVIEWER_OT_import_archive(bpy.types.Operator, ImportHelper):
        bl_idname = 'polyviewer.import_archive'
        bl_label = 'Import PolyViewer scene.json'
        filename_ext = '.json'
        filter_glob: bpy.props.StringProperty(default='scene.json', options={'HIDDEN'})

        def execute(self, context):
            try:
                path = Path(self.filepath).resolve()
                if path.name != 'scene.json':
                    raise RuntimeError('Select scene.json from the fully extracted PolyViewer ZIP.')
                import_archive(path.parent)
            except Exception as error:
                import traceback
                traceback.print_exc()
                self.report({'ERROR'}, str(error))
                return {'CANCELLED'}
            self.report({'INFO'}, 'PolyViewer scene imported. Save as .blend to keep it.')
            return {'FINISHED'}

    previous = getattr(bpy.types, 'POLYVIEWER_OT_import_archive', None)
    if previous is not None:
        bpy.utils.unregister_class(previous)
    bpy.utils.register_class(POLYVIEWER_OT_import_archive)


space = bpy.context.space_data
text = getattr(space, 'text', None) if getattr(space, 'type', None) == 'TEXT_EDITOR' else None
archive_root = find_archive(globals().get('__file__'), getattr(text, 'filepath', None))
if archive_root is not None:
    globals().update(import_archive(archive_root))
elif bpy.app.background:
    raise RuntimeError('PolyViewer scene.json was not found. Extract the entire ZIP and run its import_scene.py with an absolute path.')
else:
    register_archive_picker()
    bpy.ops.polyviewer.import_archive('INVOKE_DEFAULT')
