...
- A compact right-side Replay dropdown contains import, native visibility, per-car opacity, rename/remove controls, searchable run names, and an independent name-label toggle for every car including the main replay. Its viewport-bounded list keeps a thin scrollbar visible at the far right and remains usable when later performance work raises capacity toward 200 cars.
