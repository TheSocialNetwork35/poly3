...
- Imported worker states remain in their original compact transferred byte representation and are decoded on demand into two reusable state objects instead of expanding every millisecond into a permanent graph of JavaScript objects. The bridge visits every distinct retained state in chronological order, avoiding repeated millisecond lookups of the same sampled state, but runs the expensive native `Car.update` only once per displayed or exported frame, matching PolyTrack's own visual cadence rather than updating the full model up to 1,000 times per replay-second.
...
Enabled replay name labels appear from frame zero and follow the final render camera, without the native race-start or 2.5–50 m distance gates. Hidden cars still hide their labels. Label refresh traverses active runtime cars directly, and disabled labels skip canvas/texture creation; toggling, renaming and rebuilding a label refreshes its visibility immediately.

During deterministic capture the normal game-state animation update is suspended, preventing duplicate car updates while the encoder is pending. The animation clock continues ticking, so preview resumes without a large time jump. Physics simulation and exported model fidelity are unchanged. Regression tests compare dense/sampled state traversal and demonstrate 4,000 instead of 34,000 state lookups for a 17 ms sampled step across 2,000 cars; this is a work-count reduction, not an end-to-end export speed claim.
...
- **Blender scene (ZIP + importer)**: select range and FPS, extract the entire downloaded ZIP, double-click `Import-Blender.command` on macOS or `Import-Blender.cmd` on Windows. The launcher finds an installed Blender (4.2+) and imports automatically in a new Blender process. For custom installations, set `BLENDER_EXECUTABLE` to the Blender executable. If macOS loses the executable permission, run `bash` followed by the dragged-in `.command` file in Terminal. Alternatively, open `import_scene.py` in Blender's Scripting workspace and run it. If Blender has lost the script's original path, the importer opens a file browser: select `scene.json` in the fully extracted export folder. The importer creates a new scene without clearing your existing scene. Save it as `.blend`; images are packed and animations use ordinary Blender keyframes, without a playback handler or external runtime dependency.

**✦ Shaders** opens the new look panel (also available with Home), with the PolyProcessing effect set, contact shadows and saved presets. Bloom, nine tone operators, colour grading, depth of field, SMAA/FXAA/MSAA and creative effects work in an optional live preview and in PNG/MP4 capture. Preview starts off; select **Use shader settings** in Render to apply the current look to an export. Shader code and GPU passes are loaded only when requested, and disabling preview restores native rendering. All passes share the same camera for stable camera moves. This is raster postprocessing, not path tracing. See [shader settings and architecture](docs/shaders.md).

Blender lighting is explicit: **PolyViewer World → Background → Strength** starts at **0**; the exported sun is in **PolyViewer Lighting**. **PolyViewer Car Lights** contains two front spotlights and one red rear brake light per visible car, attached to its animated chassis. Brake power switches with the recorded S / down-arrow input, using constant keyframes on the actual Area light (0 / 300 W). The rear material stays static. Its lens follows the original sloped BrakeLight surface with 1 mm outward clearance; the Area light shares that surface orientation. Imported car parts use flat shading without custom smoothing normals. Front spotlights default to 1,500 W each. Front lens glow follows lamp power. Lamp powers can be changed near the top of `import_scene.py` before importing. Re-export the ZIP to include the new car/brake metadata; older archives still import, but cannot acquire reliable brake animation from the new script alone. Material Preview uses Scene World/Scene Lights; Solid mode still uses Blender's studio lighting.

Blender exports bake evaluated world transforms, every track/building instance, wheels, suspension morphs, camera pose and vertical FOV at the chosen output FPS. The original Camera Point data is also retained on the Blender scene. Meshes, vertex colors, UVs, texture transforms, material groups, sampled visibility, native smoke cards and evolving skid geometry are included. Native projected-shadow meshes are omitted entirely to avoid ground z-fighting; Blender lights cast real shadows. Static geometry is shared, and frame files store only changed objects. Geometry/material/texture resources are streamed as separate files under resources/ instead of one giant scene.json; only drawn vertices are retained. There is no fixed 512 MiB export budget; available browser memory and ZIP format limits still apply. Keep resources/ alongside scene.json and frames/ when importing.
