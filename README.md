...
- Preview and export share one `SceneEvaluator` for Camera Point evaluation. The adaptive native replay bridge keeps one-millisecond history for priority cars (main run, current camera target, then marked runs) and directly evaluates the exact timestamp for the remaining crowd. All cars retain the real PolyTrack model, materials, wheel state, replay transform, and renderer path.
- Adaptive quality is automatic: 1–20 visible cars use Full mode, 21–100 use Balanced, 101–250 use Crowd, and larger scenes use Massive mode. Cars created beyond the first 20 avoid allocating their own expensive particle and skidmark systems; non-priority crowd cars skip independent replay-camera updates and shadow work. Hidden cars are removed from visual evaluation as well as rendering.
...
The current safe product ceiling is 500 imported cars, not the eventual thousands-car goal. Full browser qualification on representative hardware is still required at 20/50/100/250/500 before raising that ceiling; the adaptive architecture and hot-path reductions are implemented now. Project save/load is not yet presented as finished. Video export is available only when the current browser exposes a compatible WebCodecs encoder; unsupported browsers receive an explicit error instead of a fake realtime fallback.
