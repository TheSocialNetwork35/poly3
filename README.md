...
- Preview and export share one `SceneEvaluator` for Camera Point evaluation. The interactive preview updates and draws at most 20 enabled cars so navigation stays responsive; the active camera target is always included. Final rendering automatically switches to every enabled car and restores the preview budget after completion or cancellation. Every displayed or exported replay retains the real PolyTrack model, materials, wheels, suspension, shadows, particles, skidmarks, replay transform, and renderer path—there are no reduced-quality car tiers.
...
The current product ceiling is 500 imported cars. Editing previews at most 20 enabled cars at full fidelity, while deterministic export includes all enabled cars and may take longer than realtime on ordinary laptops. Full hardware qualification and a style-aware draw-call batching layer are still required before raising the ceiling toward thousands. Project save/load is not yet presented as finished. Video export is available only when the current browser exposes a compatible WebCodecs encoder; unsupported browsers receive an explicit error instead of a fake realtime fallback.
